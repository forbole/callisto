...
func saveProposals(
	...
	proposals := make([]bgovtypes.Proposal, len(p))
	tallyResults := make([]bgovtypes.TallyResult, len(p))
	deposits := make([]bgovtypes.Deposit, len(p))

	for index, proposal := range p {
		// Since it's not possible to get the proposer, set it to nil
		proposals[index] = bgovtypes.NewProposal(
			proposal.GetTitle(),
		...
		tallyResults[index] = bgovtypes.NewTallyResult(
			proposal.ProposalId,
		...
		deposits[index] = bgovtypes.NewDeposit(
			proposal.ProposalId,
			"",
