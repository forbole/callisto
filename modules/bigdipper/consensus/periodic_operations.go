...
func Register(scheduler *gocron.Scheduler, db *bigdipperdb.Db) error {
	log.Debug().Str("module", "consensus").Msg("setting up periodic tasks")

	if _, err := scheduler.Every(1).Minute().Do(func() {
		utils2.WatchMethod(func() error { return updateBlockTimeInMinute(db) })
	}); err != nil {
		return err
	}

	if _, err := scheduler.Every(1).Hour().StartImmediately().Do(func() {
		utils2.WatchMethod(func() error { return updateBlockTimeInHour(db) })
	}); err != nil {
		return err
	}

	if _, err := scheduler.Every(1).Day().StartImmediately().Do(func() {
		utils2.WatchMethod(func() error { return updateBlockTimeInDay(db) })
	}); err != nil {
		return err
	}
...
func updateBlockTimeInMinute(db *bigdipperdb.Db) error {
...
func updateBlockTimeInHour(db *bigdipperdb.Db) error {
	log.Debug().Str("module", "consensus").Str("operation", "block time").
...
func updateBlockTimeInDay(db *bigdipperdb.Db) error {
	log.Debug().Str("module", "consensus").Str("operation", "block time").
