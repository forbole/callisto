...
// saveParams saves the given params into the bigdipper
func saveParams(params stakingtypes.Params, db *bigdipperdb.Db) error {
	return db.SaveStakingParams(bstakingtypes.NewStakingParams(
		params.BondDenom,
	))
}
...
func saveValidators(
	doc *tmtypes.GenesisDoc, validators stakingtypes.Validators, cdc codec.Marshaler, db *bigdipperdb.Db,
) error {
	vals := make([]bstakingtypes.Validator, len(validators))
	for i, val := range validators {
		validator, err := bstakingcommon.ConvertValidator(cdc, val, doc.InitialHeight)
		if err != nil {
			return err
		}
...
func saveDelegations(doc *tmtypes.GenesisDoc, genState stakingtypes.GenesisState, db *bigdipperdb.Db) error {
...
func saveUnbondingDelegations(doc *tmtypes.GenesisDoc, genState stakingtypes.GenesisState, db *bigdipperdb.Db) error {
...
func saveRedelegations(doc *tmtypes.GenesisDoc, genState stakingtypes.GenesisState, db *bigdipperdb.Db) error {
