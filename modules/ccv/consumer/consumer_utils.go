...
func (m *Module) UpdateCcvValidators(height int64) error {
	...
	for _, index := range validatorsDB {
		...
		if len(providerSelfDelegateAddress) > 0 {
			...
			consumerSelfDelegateAddress, err := bech32.ConvertAndEncode("stride", bz)
			if err != nil {
				return fmt.Errorf("error while encoding consumer self delegate address: %s", err)
			}

			consumerOperatorAddress, err := bech32.ConvertAndEncode("stridevaloper", bz)
			if err != nil {
			...
			providerConsensusAddress, err = m.providerModule.GetValidatorProviderAddr(providerLatestHeight, "stride-1", providerConsensusAddress)
			if err != nil {
