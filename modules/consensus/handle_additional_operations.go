...
import (
	...
	types2 "github.com/forbole/bdjuno/types"
...
func ListenOperation(cp *client.Proxy, db *database.Db) {
	events := []string{
...
func handleEvent(event tmctypes.ResultEvent, db *database.Db) {
...
func mapEvent(event tmctypes.ResultEvent) *types2.ConsensusEvent {
	switch data := event.Data.(type) {
	case tmtypes.EventDataNewRound:
		return types2.NewConsensusEvent(data.Height, data.Round, data.Step)
	...
	case tmtypes.EventDataVote:
		return types2.NewConsensusEvent(data.Vote.Height, data.Vote.Round, tmtypes.EventVote)
