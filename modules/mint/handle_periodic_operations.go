...
func (m *Module) UpdateInflation() error {
	...
	// Get the inflation
	resp, err := http.Get("https://api.coreum.forbole.com/cosmos/mint/v1beta1/inflation")
	if err != nil {
		return fmt.Errorf("error while querying coreum API for inflation value: %s", err)
	}
