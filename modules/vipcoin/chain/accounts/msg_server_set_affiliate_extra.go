...
func (m *Module) handleMsgSetAffiliateExtra(tx *juno.Tx, index int, msg *types.MsgSetAffiliateExtra) error {
	...
	acc, err := m.accountRepo.GetAccounts(filter.NewFilter().SetArgument(FieldHash, msg.AccountHash))
	if err != nil {
		return err
	}

	affAcc, err := m.accountRepo.GetAccounts(filter.NewFilter().SetArgument(FieldHash, msg.AffiliationHash))
	if err != nil {
		return err
	}
