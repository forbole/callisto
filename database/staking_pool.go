...
func (db *Db) SaveStakingPool(pool *types.Pool) error {
	stmt := `
INSERT INTO staking_pool (bonded_tokens, not_bonded_tokens, unbonding_tokens, height) 
VALUES ($1, $2, $3)
...
WHERE staking_pool.height <= excluded.height`
	...
	_, err := db.Sql.Exec(stmt, pool.BondedTokens.String(), pool.NotBondedTokens.String(), pool.UnbondingTokens.String(), pool.Height)
	if err != nil {
