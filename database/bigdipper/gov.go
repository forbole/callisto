...
func (db *Db) SaveProposals(proposals []bgovtypes.Proposal) error {
	//do nothing if empty
	if len(proposals) == 0 {
		return nil
	}
...
func (db *Db) SaveProposal(proposal bgovtypes.Proposal) error {
	query := `
INSERT INTO proposal(
	title, description, proposer_address, proposal_route, proposal_type, proposal_id, status, 
    submit_time, deposit_end_time, voting_start_time, voting_end_time
...
func (db *Db) SaveTallyResults(tallys []bgovtypes.TallyResult) error {
	if len(tallys) == 0 {
		return nil
	}
...
func (db *Db) SaveVote(vote bgovtypes.Vote) error {
...
func (db *Db) SaveDeposit(deposit bgovtypes.Deposit) error {
	query := `
...
func (db *Db) SaveDeposits(deposits []bgovtypes.Deposit) error {
	if len(deposits) == 0 {
		return nil
	}
