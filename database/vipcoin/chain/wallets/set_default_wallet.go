...
// SaveDefaultWallets - inserts messages into the "overgold_chain_wallets_set_default_wallet" table
func (r Repository) SaveDefaultWallets(messages *walletstypes.MsgSetDefaultWallet, transactionHash string) error {
	query := `INSERT INTO overgold_chain_wallets_set_default_wallet 
...
// GetDefaultWallets - get the given messages from the "overgold_chain_wallets_set_default_wallet" table
func (r Repository) GetDefaultWallets(filter filter.Filter) ([]*walletstypes.MsgSetDefaultWallet, error) {
	query, args := filter.Build("overgold_chain_wallets_set_default_wallet", types.FieldCreator, types.FieldAddress)
