...
func (db *Db) SaveValidatorsStatuses(statuses []types.ValidatorStatus) error {
	...
	statusStmt := `INSERT INTO validator_status (validator_address, status, jailed, liquid_validator, height) VALUES `
	...
	for i, status := range statuses {
		...
		statusParams = append(statusParams, status.ConsensusAddress, status.Status, status.Jailed, status.LiquidValidator, status.Height)
	}
