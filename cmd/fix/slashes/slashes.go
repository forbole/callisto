...
// FixSlashesCmd returns a Cobra command that allows to fix the delegations for all the slashed validators.
func FixSlashesCmd(parseConfig *parse.Config) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, validator := range validators {
				// If the validator is not jailed, skip it
				if !validator.Jailed {
					continue
				}
