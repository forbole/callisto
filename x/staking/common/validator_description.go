...
func GetValidatorDescription(opAddr string, description stakingtypes.Description) (types.ValidatorDescription, error) {
	avatarURL, err := keybase.GetAvatarURL(description.Identity)
	if err != nil {
		return types.ValidatorDescription{}, err
	}

	return types.NewValidatorDescription(opAddr, description, avatarURL), nil
}
