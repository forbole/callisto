...
func (k Keeper) MintNFTUnverified(ctx sdk.Context, denomID, tokenID, tokenNm, tokenDn, tokenURI string, tokenTags []string, owner sdk.AccAddress) error {
...
func (k Keeper) MintNFT(
	ctx sdk.Context, denomID, tokenID, tokenNm, tokenDn,
	tokenURI string, tokenTags []string, sender, owner sdk.AccAddress,
) error {
	...
	return k.MintNFTUnverified(ctx, denomID, tokenID, tokenNm, tokenDn, tokenURI, tokenTags, owner)
}

// EditNFT updates an already existing NFT
func (k Keeper) EditNFT(
	ctx sdk.Context, denomID, tokenID, tokenNm, tokenDn,
	tokenURI string, tokenTags []string, owner sdk.AccAddress,
) error {
	if !k.HasDenomID(ctx, denomID) {
		return sdkerrors.Wrapf(types.ErrInvalidDenom, "denom ID %s not exists", denomID)
	}
	...
	if types.Modified(tokenDn) {
		nft.Description = tokenDn
	}
	...
	if types.StringArrayModified(tokenTags) {
		nft.Tags = tokenTags
	}
