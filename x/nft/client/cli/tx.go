...
func GetCmdMintNFT() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tokenDescription, err := cmd.Flags().GetString(FlagTokenDescription)
			if err != nil {
				return err
			}
			tokenFileURI, err := cmd.Flags().GetString(FlagTokenFileURI)
			if err != nil {
				return err
			}
			tokenTags, err := cmd.Flags().GetStringSlice(FlagTokenTags)
			if err != nil {
				return err
			}
